...
function Lessons({ section, lessons }: Props) {
	...
	const onAdd = async (lesson: ILessonFields) => {
		setIsLoading(true)
		return createLesson({ lesson, section: section._id, path }).then(() => onToggle()).finally(() => setIsLoading(false))
	}
	...
	const onEdit = async (lesson: ILessonFields) => {
		setIsLoading(true)
		return editLesson(lesson, lessonId, path).then(() => onFinishEdit()).finally(() => setIsLoading(false))
	}
		...
		<Card>
			<CardContent className='relative p-6'>
				...
				{state ? (
					...
					<Forms lesson={currentLesson as ILessonFields} handler={onEdit} isEdit onCancel={onFinishEdit} />
				) : (
					<>
						{!lessons.length ? (
